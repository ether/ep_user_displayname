# Contributing to ep_user_displayname

Thanks for helping improve `ep_user_displayname` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_user_displayname` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_user_displayname from your local clone
pnpm run plugins i --path ../ep_user_displayname

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_user_displayname/` are picked up after a server restart.
...
* Etherpad version, ep_user_displayname version, browser + OS
* Steps to reproduce
