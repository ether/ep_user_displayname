# Agent Guide — ep_user_displayname
...
`ep_user_displayname` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_user_displayname`:
...
* Server: `clientVars`, `handleMessage`, `init_ep_user_displayname`, `loadSettings`
* Client: `postAceInit`
